<script lang="ts">
  import type { Contact } from "$lib/state/chat.svelte";
  import MessageItem from "$lib/components/MessageItem.svelte";

  const { selectedContact }: {selectedContact: Contact} = $props();

  let currentMessage = $state('');

</script>

<div class="flex flex-col h-full w-full">
    <div class="flex flex-row h-full flex-1">
        <div class="flex flex-col flex-auto flex-shrink-0  bg-gray-100 h-full">
            <!-- Header -->
            <header class="flex flex-row items-center justify-start gap-x-2 bg-gray-300 p-4">
                <div class={`flex items-center justify-center h-8 w-8 bg-gray-200 rounded-full font-sans font-bold`}>
                    {selectedContact.initialToUpper}
                </div>
                <div class="font-regular text-lg font-sans">
                    {selectedContact.name}
                </div>
                <h1>
                    {selectedContact.status.toUpperCase()}
                </h1>
            </header>
            <!-- Messages -->
            <main class="flex flex-col h-full overflow-x-auto mb-4">
                <div class="flex flex-col h-full">
                    <div class="">
                        {#each selectedContact.messages as message}
                            <MessageItem {message} isOwnMessage={message.sender === 'me'} />
                        {/each}
                    </div>
                </div>
            </main>
            <!-- Input -->
            <footer class="flex flex-row items-center h-16 rounded-xl bg-white w-full px-4">
                <div>
                    <!-- svelte-ignore a11y_consider_explicit_label -->
                    <button class="flex items-center justify-center text-gray-400 hover:text-gray-600">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.172 7l-6.586 6.586a2 2 0 102.828 2.828l6.414-6.586a4 4 0 00-5.656-5.656l-6.415 6.585a6 6 0 108.486 8.486L20.5 13"></path>
                        </svg>
                    </button>
                </div>
                <div class="flex-grow ml-4">
                    <div class="relative w-full">
                        <input 
                        bind:value={currentMessage}
                        onkeydown={(e) => {
                            if (e.key === 'Enter') {
                                selectedContact.addTextMessage(currentMessage, true);
                                currentMessage = '';
                                e.currentTarget!.value = '';
                            }                            
                          }}
                        type="text"  class="flex w-full border rounded-xl focus:outline-none focus:border-indigo-300 pl-4 h-10" />
                        <!-- svelte-ignore a11y_consider_explicit_label -->
                        <button class="absolute flex items-center justify-center h-full w-12 right-0 top-0 text-gray-400 hover:text-gray-600">
                            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14.828 14.828a4 4 0 01-5.656 0M9 10h.01M15 10h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                            </svg>
                        </button>
                    </div>
                </div>
                <!-- Send button -->
                <div class="ml-4">
                    <button 
                    onclick={() => selectedContact.addTextMessage(currentMessage, true)}
                    class="flex items-center justify-center bg-indigo-500 hover:bg-indigo-600 rounded-xl text-white px-4 py-1 flex-shrink-0">
                        <span>Send</span>
                        <span class="ml-2">
                            <svg class="w-4 h-4 transform rotate-45 -mt-px" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 19l9 2-9-18-9 18 9-2zm0 0v-8"></path>
                            </svg>
                        </span>
                    </button>
                </div>
            </footer>
        </div>
    </div>
</div>